...
def pearson_1d(x, y):
    ...
    mc_x=x-np.mean(x)
    mc_y=y-np.mean(y)
    #a=np.sum(mc_x.dot(mc_y))
    #b=np.sum(mc_x.dot(mc_x))
    #c=np.sum(mc_y.dot(mc_y))
...
def pearson_2d(x, Y):
    ...
    # +++your code here+++
    mc_x = x - np.mean(x)
    # mc_Y=np.ones((Y.shape[0],1)) * np.mean(axis=0).reshape(1, Y.shape[1])
    #mc_y = Y - np.outer(np.ones((Y.shape[0], 1)), np.mean(Y, axis=0).reshape(1,Y.shape[1]))
